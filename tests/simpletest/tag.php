<?php
     ...
     *	@version	$Id: tag.php,v 1.1 2006/01/09 15:11:38 malyvelky Exp $
     */
    ...
    class SimpleOptionTag extends SimpleWidget {
        ...
        function getDefault() {
            ...
            // SimpleTag::getAttribute returnes true if value=""
            // but that's a nonsense for an option - in such a case we 
            // must return the empty string (btw, true gets converted into 1):
    ...
    class SimpleRadioGroup {
        ...
        function getValue() {
            for ($i = 0, $count = count($this->_widgets); $i < $count; $i++) {
            	$value = $this->_widgets[$i]->getValue();
                if (isset($value) && $value!==false) {// Jakub Holy: modified not to convert value="0" to false
