          ...
          <table border="1" cellspacing="0" cellpadding="3">
            ...
            {section name="i" loop=$data.invite show=TRUE}
              <tr{section name="erruid" loop=1 show=$data.invite[i].erruid} style="background-color:red"{/section}>
                ...
                <td align="center">
                  {section name="mc" loop=1 show=$data.show.manual_codes}
                    <input type="text" name="invite_code[{$data.invite[i].uid}]" size="21" maxlength="20" value="{$data.invite[i].invite_code}"/>
                  {sectionelse}
                    {$data.invite[i].invite_code}
                  {/section}
                </td>
